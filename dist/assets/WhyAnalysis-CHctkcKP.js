import{V as se,Y as ae,r as c,O as e,a3 as N,S as G,ah as ne,X as re,aa as H}from"./index-vbTRhif2.js";import{M as ie}from"./MindMap-B8PUT4NP.js";import{A as L}from"./alert-circle-BjjWgJEF.js";import"./zoom-out-D8o2hOcw.js";import"./trash-2-VPxR214V.js";const he=()=>{var E;const{id:K}=se(),{a3Cases:V,updateA3Case:m}=ae(),t=V.find(s=>s.id===K),[x,w]=c.useState(""),[j,S]=c.useState(!1),[A,y]=c.useState(null),[C,k]=c.useState(null),[M,I]=c.useState(!1),[O,b]=c.useState(null);c.useEffect(()=>{t&&t.rootCause!==x&&w(t.rootCause||"")},[t]),c.useEffect(()=>{if(!(t!=null&&t.mindMapNodes))return;const s=t.mindMapNodes;if(s.length===0)return;const n=s.filter(a=>!a.parentId);let r="";const i=(a,o)=>{const h=s.filter(l=>l.parentId===a);h.sort((l,d)=>l.y-d.y);for(const l of h){const d="  ".repeat(o);r+=`${d}- ${l.text}
`,i(l.id,o+1)}};n.sort((a,o)=>a.y-o.y);for(const a of n)r+=`${a.text}
`,i(a.id,1);r!==t.mindMapText&&m({...t,mindMapText:r})},[t==null?void 0:t.mindMapNodes,m,t==null?void 0:t.id]);const X=c.useCallback(s=>{if(!t)return;const n=JSON.stringify(t.mindMapNodes),r=JSON.stringify(s);n!==r&&m({...t,mindMapNodes:s})},[t,m]),q=c.useCallback(s=>{if(!t)return;const n=s.scale,r=s.height,i=t.mindMapScale??1,a=t.mindMapCanvasHeight;i===n&&a===r||m({...t,mindMapScale:n,mindMapCanvasHeight:r})},[t,m]),z=s=>{const n=s.target.value;w(n),t&&m({...t,rootCause:n})},B=async()=>{var i,a,o;if(!t)return;const s=t.problemStatement||"",n=t.dataAnalysisObservations||"",r=t.problemContext||"";if(s.trim()){I(!0),b(null);try{const h=`
