import{W as V,Z as q,r as h,O as t,a6 as O,an as K,S as T,ai as B,X as U,ao as Z,ae as W}from"./index-CQJl7MZx.js";const re=()=>{const{id:P}=V(),{a3Cases:Y,updateA3Case:o}=q(),e=Y.find(s=>s.id===P),[g,y]=h.useState(""),[v,N]=h.useState(!1),[j,p]=h.useState(null);h.useEffect(()=>{e&&e.rootCause!==g&&y(e.rootCause||"")},[e]),h.useEffect(()=>{if(!(e!=null&&e.mindMapNodes))return;const s=e.mindMapNodes;if(s.length===0)return;const n=s.filter(a=>!a.parentId);let r="";const l=(a,d)=>{const x=s.filter(c=>c.parentId===a);x.sort((c,m)=>c.y-m.y);for(const c of x){const m="  ".repeat(d);r+=`${m}- ${c.text}
