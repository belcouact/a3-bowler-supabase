import{$ as V,a1 as q,r as h,O as t,a9 as O,Q as K,S as T,ak as B,X as Q,as as U,ah as P}from"./index-D53RQmFX.js";const re=()=>{const{id:W}=V(),{a3Cases:Y,updateA3Case:o}=q(),e=Y.find(s=>s.id===W),[g,y]=h.useState(""),[v,N]=h.useState(!1),[j,p]=h.useState(null);h.useEffect(()=>{e&&e.rootCause!==g&&y(e.rootCause||"")},[e]),h.useEffect(()=>{if(!(e!=null&&e.mindMapNodes))return;const s=e.mindMapNodes;if(s.length===0)return;const n=s.filter(a=>!a.parentId);let r="";const l=(a,d)=>{const x=s.filter(c=>c.parentId===a);x.sort((c,m)=>c.y-m.y);for(const c of x){const m="  ".repeat(d);r+=`${m}- ${c.text}
