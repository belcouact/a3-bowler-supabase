import{$ as V,a1 as q,r as h,O as t,a9 as O,Q as K,S as T,ak as B,X as Q,as as U,ah as P}from"./index-9EAd4Y7y.js";const re=()=>{const{id:W}=V(),{a3Cases:Y,updateA3Case:o}=q(),e=Y.find(s=>s.id===W),[g,y]=h.useState(""),[v,N]=h.useState(!1),[j,p]=h.useState(null);h.useEffect(()=>{e&&e.rootCause!==g&&y(e.rootCause||"")},[e]),h.useEffect(()=>{if(!(e!=null&&e.mindMapNodes))return;const s=e.mindMapNodes;if(s.length===0)return;const n=s.filter(a=>!a.parentId);let r="";const l=(a,d)=>{const x=s.filter(c=>c.parentId===a);x.sort((c,m)=>c.y-m.y);for(const c of x){const m="  ".repeat(d);r+=`${m}- ${c.text}
`,l(c.id,d+1)}};n.sort((a,d)=>a.y-d.y);for(const a of n)r+=`${a.text}
`,l(a.id,1);r!==e.mindMapText&&o({...e,mindMapText:r})},[e==null?void 0:e.mindMapNodes,o,e==null?void 0:e.id]);const J=h.useCallback(s=>{if(!e)return;const n=JSON.stringify(e.mindMapNodes),r=JSON.stringify(s);n!==r&&o({...e,mindMapNodes:s})},[e,o]),$=h.useCallback(s=>{if(!e)return;const n=s.scale,r=s.height,l=e.mindMapScale??1,a=e.mindMapCanvasHeight;l===n&&a===r||o({...e,mindMapScale:n,mindMapCanvasHeight:r})},[e,o]),I=s=>{const n=s.target.value;y(n),e&&o({...e,rootCause:n})},R=async()=>{var l,a,d;if(!e)return;const s=e.problemStatement||"",n=e.dataAnalysisObservations||"",r=e.problemContext||"";if(s.trim()){N(!0),p(null);try{const x=`
