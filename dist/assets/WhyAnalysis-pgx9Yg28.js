import{V as ne,Y as ae,r as d,O as e,a3 as N,S as R,ak as re,al as ie,X as oe,ac as G}from"./index-IbZS1YUW.js";import{A as H}from"./alert-circle-DUN51T_Q.js";const pe=()=>{var E;const{id:L}=ne(),{a3Cases:X,updateA3Case:h}=ae(),t=X.find(s=>s.id===L),[p,w]=d.useState(""),[j,S]=d.useState(!1),[A,y]=d.useState(null),[C,k]=d.useState(null),[M,I]=d.useState(!1),[O,b]=d.useState(null);d.useEffect(()=>{t&&t.rootCause!==p&&w(t.rootCause||"")},[t]),d.useEffect(()=>{if(!(t!=null&&t.mindMapNodes))return;const s=t.mindMapNodes;if(s.length===0)return;const a=s.filter(n=>!n.parentId);let r="";const i=(n,o)=>{const u=s.filter(l=>l.parentId===n);u.sort((l,m)=>l.y-m.y);for(const l of u){const m="  ".repeat(o);r+=`${m}- ${l.text}
