...
${r||"(none provided)"}
`,i=await fetch("https://multi-model-worker.study-llm.me/api/chat",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"deepseek",messages:[{role:"system",content:"You are an A3 Problem Solving coach that outputs strictly JSON in the requested schema."},{role:"user",content:l}],stream:!1})});if(!i.ok)throw new Error("Failed to generate why analysis");const B=(((a=(o=(n=(await i.json()).choices)==null?void 0:n[0])==null?void 0:o.message)==null?void 0:a.content)||"{}").replace(/```json/gi,"").replace(/```/g,"").trim();let g;try{g=JSON.parse(B)}catch{g={}}const P=Array.isArray(g.whyTree)?g.whyTree:[];if(P.length===0){b("AI did not return a valid why tree. Please try again.");return}const v=[],F=G();v.push({id:F,text:s,x:50,y:200,width:260,height:100,parentId:null,type:"root"});const Y=(D,U,T,Q)=>{let p=Q;const W=120,Z=220;return D.forEach(m=>{if(!m||typeof m.cause!="string"||!m.cause.trim())return;const J=G();v.push({id:J,text:m.cause.trim(),x:50+Z*T,y:p,parentId:U,type:"child"}),Array.isArray(m.children)&&m.children.length>0?p=Y(m.children,J,T+1,p+W):p+=W}),p};Y(P,F,1,260),d({...t,mindMapNodes:v})}catch{b("Failed to generate why analysis. Please try again.")}finally{I(!1)}}},X=async()=>{var o,a,l;if(!t)return;const s=t.problemStatement||"",r=t.dataAnalysisObservations||"",n=x||t.rootCause||"";if(!(!s.trim()||!n.trim())){S(!0),k(null),y(null);try{const h=[{role:"system",content:`You are an expert continuous improvement and operations coach.
